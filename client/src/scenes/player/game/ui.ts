    ...
    static render() {
        ...
        if (!uiLayer) {
            ...
            uiLayer.innerHTML = `
                ...
                <div id="analog-container" class="fixed bottom-6 left-6 z-[999] opacity-80 md:hidden pointer-events-auto">
                    <div id="joystick-base" class="relative w-24 h-24 rounded-full border-4 border-[#72BF78] bg-black/40 backdrop-blur-sm touch-none">
                        <div id="joystick-stick" class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#72BF78] transition-transform duration-100 ease-out"></div>
                    </div>
                </div>
            `;
